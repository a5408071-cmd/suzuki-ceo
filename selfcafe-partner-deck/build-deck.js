...
function shell(s, secNo, secName, title, lead, pageNo) {
...
/* ===================================================== p1 表紙 */
{
  ...
  const stats = [
    ...
    { x: 7.389, w: 2.5, v: "650", u: "万円〜", l: "開業資金（居抜き20坪想定）" },
  ];
  stats.forEach((st) => {
    s.addText(
      [
...
{
  const s = pres.addSlide();
  shell(s, 1, "会社概要", "会社概要", "無人カフェ「セルフカフェ」の企画・開発・運営を行っています。", 2);
...
{
  const s = pres.addSlide();
  shell(s, 2, "市場背景", "社会背景と市場性", "現代社会が抱える3つの課題が、無人カフェの需要をつくっています。", 3);
...
{
  const s = pres.addSlide();
  shell(s, 3, "優位性", "人手不足対策：無人管理の優位性", "運営から「人」の変数を外すと、コスト・品質・安定性が同時に改善します。", 4);
...
{
  const s = pres.addSlide();
  shell(s, 4, "不動産活用", "持ち物件オーナー様へ", "自社物件なら家賃コストがゼロ。売上をダイレクトに利益へ繋げられます。", 5);

  chipRow(s, TOP, [
    { v: "0", u: "円", l: "自社物件なら新規の家賃負担なし" },
    { v: "20", u: "坪〜", l: "必要面積の目安（40席想定）" },
    { v: "650", u: "万円〜", l: "開業資金（居抜き・20坪想定）" },
...
{
  const s = pres.addSlide();
  shell(s, 5, "ブランド", "セルフカフェとは",
    "“誰もが気軽に使える”第三の居場所を、全国66店舗で展開中。年内にさらに10店舗以上の出店を予定しています。", 6);
...
{
  const s = pres.addSlide();
  shell(s, 6, "実績データ", "店舗展開数の推移",
    "2022年9月の1号店OPENから、驚異的なスピードで出店エリアを拡大。出店は今後も継続していきます。", 7);
...
{
  const s = pres.addSlide();
  shell(s, 7, "実績データ", "月間利用者数の推移",
    "ブランドの認知拡大とともに、月間の利用者数は約10万人規模まで拡大しました。", 8);
  ...
  s.addChart(pres.ChartType.bar, [{ name: "月間利用者数", labels, values }], {
    ...
    valAxisLabelColor: C.muted, valAxisLabelFontFace: F.num, valAxisLabelFontSize: 9,
    valAxisLabelFormatCode: "#,##0", valAxisMinVal: 0, valAxisMaxVal: 120000, valAxisMajorUnit: 40000,
    valAxisLineShow: false, valGridLine: { color: C.warmLine, size: 0.75 },
  });
  ...
  tiles.forEach((t, i) => {
    ...
    s.addText(t[2], {
      x: rx + 0.26, y: y + 0.7, w: rw - 0.52, h: 0.4,
      fontFace: F.jp, fontSize: 9.5, color: dark ? C.cvBody : C.muted, margin: 0, valign: "top", lineSpacingMultiple: 1.2,
    });
  });
...
{
  const s = pres.addSlide();
  shell(s, 8, "利用システム", "利用システム：究極の利便性と価格",
    "ドリンク1杯で、時間制限なし。だから毎日使われ、リピートが積み上がります。", 9);

  const items = [
    ["LuCoffee", "390円〜", "1杯あたり", "ドリンク一杯で入店",
...
{
  const s = pres.addSlide();
  shell(s, 9, "ユーザー層", "セルフカフェのユーザー層",
    "「勉強」を目的に、若年層が繰り返し来店する——長時間・高頻度の利用構造です。", 10);
...
{
  const s = pres.addSlide();
  shell(s, 12, "ポジショニング", "比較表：いいところ取りのビジネスモデル",
    "カフェの「気軽さ」とオフィスの「機能」、その両方を満たす選択肢がありませんでした。", 13);
...
{
  const s = pres.addSlide();
  shell(s, 13, "運営システム", "独自の無人開閉店システム",
    "警備会社と連携した安心の自動化。オーナー様が現地に足を運ぶ必要はありません。", 14);
...
{
  const s = pres.addSlide();
  shell(s, 14, "運営システム", "店内モニタリング＆サポート基盤",
    "緊急時に強い、安心のサポート体制で無人運営を支えます。", 15);

  const pw = 5.6;
  panel(s, M, TOP, pw, 3.0);
  ...
  s.addText("店内の各所に非常ボタンを設置し、お客様同士のトラブルまで幅広く対応。6台のカメラによるモニタリングで、お問い合わせにも即時対応します。", {
    x: M + 0.44, y: TOP + 1.82, w: pw - 0.88, h: 1.0,
...
{
  const s = pres.addSlide();
  shell(s, 15, "パートナー制度", "パートナー制度の3つの特徴",
    "投資を抑え、既存の資産と人員のまま始められます。", 16);

  const items = [
    ["LuWallet", "POINT 01", "1/10 以下", "低初期コスト",
      "他社カフェ業態と比較し、1/10以下のコストで出店が可能。開業資金650万円〜から始められます。",
      ["加盟金 200万円", "開業資金 650万円〜"]],
  ...
  items.forEach((it, i) => {
    ...
    s.addText(it[4], {
      x: x + 0.3, y: y + 1.78, w: w - 0.6, h: 1.3,
      fontFace: F.jp, fontSize: 10.5, color: C.body, margin: 0, valign: "top", lineSpacingMultiple: 1.45,
    });
    it[5].forEach((tag, j) => {
      const tw = 0.28 + tag.length * 0.13;
      const ty2 = y + h - 0.72 + Math.floor(j / 2) * 0.36;
      const tx = x + 0.3 + (j % 2) * 1.5;
      s.addShape("roundRect", {
        x: tx, y: ty2, w: Math.min(tw, 1.4), h: 0.28, rectRadius: 0.14,
        fill: { color: C.tint }, line: { color: C.tintLine, width: 0.75 },
      });
      s.addText(tag, {
        x: tx, y: ty2, w: Math.min(tw, 1.4), h: 0.28,
        fontFace: F.jp, fontSize: 8.5, bold: true, color: C.green, align: "center", margin: 0, valign: "middle",
      });
    });
  });
  note(s, 6.62, "※ 「1/10以下」は他社カフェ業態の開業資金との比較（次ページ参照）。");
}

/* ===================================================== p17 初期費用（他社比較） */
{
  const s = pres.addSlide();
  shell(s, 16, "初期費用", "初期費用：他社カフェ業態との比較",
    "同じ「カフェ出店」でも、必要な初期投資はまったく違います。", 17);
  ...
  s.addTable(
    [
      ...
      [tl("加盟金"), mine("200万円"), oth("300万円〜"), oth("150万円〜")],
  ...
  s.addText(
    [
      ...
      { text: "1/5 〜 1/13", options: { color: C.cvPale, bold: true } },
...
{
  const s = pres.addSlide();
  shell(s, 17, "初期費用", "初期費用・売上イメージ",
    "出店方法は2パターン。居抜き・既存店併設なら初期費用を大きく抑えられます。", 18);

  const cw = (CW - 0.42) / 2;
  const pats = [
    ["① 既存店併設 / 居抜き", "750万円〜", "内装が残る区画や自社店舗の一角を活用。工事範囲を最小化できます。", true],
    ["② スケルトン工事", "1,050万円〜", "内装のない状態からの造作。レイアウトを自由に設計できます。", false],
  ];
  pats.forEach((p, i) => {
  ...
  s.addText(
    [
      ...
      { text: "750万円〜", options: { fontFace: F.num, fontSize: 15, bold: true, color: C.green } },
    ],
    { x: M + CW - 2.6, y: by + 0.18, w: 2.3, h: 0.36, align: "right", margin: 0, valign: "middle" }
  );
  const bd = [
    ["LuHandCoins", "加盟金", "200万円", ""],
...
[
  {
    page: 19, sec: 18, label: "20坪（40席）", tag: "STANDARD",
    rev: ["売上高", "1杯400円 × 67杯 × 30日", "800,000円"],
    rows: [
      ["ドリンク原料", "原料・カップ等（20%）", "-160,000円"],
    ...
    profit: "154,000円", annual: "1,848,000円",
  },
  {
    page: 20, sec: 19, label: "70坪（120席）", tag: "LARGE",
    rev: ["売上高", "1杯400円 × 125杯 × 30日", "1,500,000円"],
    rows: [
      ["ドリンク原料", "原料・カップ等（20%）", "-300,000円"],
      ["家賃", "仮定坪単価：8,000円", "-560,000円"],
      ["清掃費", "清掃パートナー様への報酬", "-40,000円"],
      ["水道光熱費", "水道15,000円／光熱85,000円（24H営業想定）", "-100,000円"],
      ["機械使用料", "ドリンクマシン38,000円×2台、決済端末5,000円×2台", "-86,000円"],
      ["セキュリティ費", "警備会社への委託料", "-20,000円"],
      ["ロイヤリティ", "一律5万円", "-50,000円"],
      ["雑費", "ゴミ回収・防犯カメラ・クラウド利用料等", "-25,000円"],
    ],
    profit: "319,000円", annual: "3,828,000円",
  },
].forEach((sim) => {
  const s = pres.addSlide();
  shell(s, sim.sec, "収益", `収益シミュレーション ／ ${sim.label}`,
    "月次の収支イメージです。売上から諸経費を差し引いた償却前営業利益を示しています。", sim.page);
  ...
  ["売上は1杯400円で算出", "家賃は仮定坪単価による試算", "ロイヤリティは月5万円の一律", "減価償却費は含まない"].forEach((t, i) => {
...
{
  const s = pres.addSlide();
  shell(s, 20, "出店事例", "出店事例：岩手県 盛岡駅前店",
    "地方都市の駅前立地。1日80杯ペースで稼働し、家賃を吸収して利益を確保しています。", 21);
...
{
  const s = pres.addSlide();
  shell(s, 21, "オプション", "有料オプション一覧",
    "必要に応じて追加できるオプションです（金額は税抜き）。SEO・MEO対策は費用なしで標準提供します。", 22);
...
{
  const s = pres.addSlide();
  shell(s, 22, "設備", "ドリンクマシンに関して",
    "マシン・備品・メンテナンスまで含めたトータルサポートパックでご提供します。", 23);
...
{
  const s = pres.addSlide();
  shell(s, 23, "FAQ", "よくある質問",
    "導入をご検討いただく際に、よくいただくご質問をまとめました。", 24);

  const qa = [
    ["準備は難しいですか？", "内装、システム導入まで本部がフルサポートするため、初めての方でも安心です。", ["内装", "システム導入", "運営研修"]],
    ["トラブル時の対応は？", "24時間体制の警備会社と遠隔監視システムにより、オーナー様が現場に駆けつける必要はほぼありません。また顧客とのLINEチャットを設けており、そこで質問対応・意見収集を行っています。", ["24時間警備", "遠隔監視", "LINEチャット"]],
    ["人材採用については？", "新しく人材を採用する必要はありません。清掃管理・原料補充はご自身でもご対応可能ですが、業務委託として委託することも可能です。", ["採用不要", "委託も可能"]],
    ["集客はどうすれば？", "数々のメディア取材や、本部によるSEO/MEO対策をはじめとしたWebマーケティングによって集客を支援します。", ["SEO対策", "MEO対策", "メディア取材"]],
  ];
  const w = (CW - 0.42) / 2, h = 2.3;
  qa.forEach(([q, a, tags], i) => {
    const x = M + (i % 2) * (w + 0.42);
    const y = TOP + Math.floor(i / 2) * (h + 0.26);
    card(s, x, y, w, h);
    s.addText("Q.", {
      x: x + 0.3, y: y + 0.24, w: 0.4, h: 0.3,
      fontFace: F.num, fontSize: 14, bold: true, color: C.gold, margin: 0, valign: "middle",
    });
    s.addText(q, {
      x: x + 0.74, y: y + 0.22, w: w - 1.04, h: 0.34,
      fontFace: F.jp, fontSize: 15, bold: true, color: C.ink, margin: 0, valign: "middle",
    });
    s.addShape("rect", { x: x + 0.3, y: y + 0.66, w: w - 0.6, h: 0.011, fill: { color: C.warmLine } });
    s.addText("A.", {
      x: x + 0.3, y: y + 0.8, w: 0.4, h: 0.28,
      fontFace: F.num, fontSize: 12, bold: true, color: C.green, margin: 0, valign: "middle",
    });
    s.addText(a, {
      x: x + 0.74, y: y + 0.78, w: w - 1.04, h: 0.94,
      fontFace: F.jp, fontSize: 10.5, color: C.body, margin: 0, valign: "top", lineSpacingMultiple: 1.4,
    });
    let tx = x + 0.74;
    tags.forEach((t) => {
      const tw = 0.26 + t.length * 0.13;
      s.addShape("roundRect", {
        x: tx, y: y + h - 0.5, w: tw, h: 0.28, rectRadius: 0.14,
        fill: { color: C.tint }, line: { color: C.tintLine, width: 0.75 },
      });
      s.addText(t, {
        x: tx, y: y + h - 0.5, w: tw, h: 0.28,
        fontFace: F.jp, fontSize: 8.5, bold: true, color: C.green, align: "center", margin: 0, valign: "middle",
      });
      tx += tw + 0.12;
    });
...
{
  const s = pres.addSlide();
  shell(s, 24, "サポート", "万全のサポート体制",
    "新たに人を採用・配置する必要はありません。開業前から開業後まで本部が伴走します。", 25);
...
{
  const s = pres.addSlide();
  shell(s, 25, "導入プロセス", "開業までの流れ",
    "お問い合わせから運営開始まで、2〜3か月程度が目安です。", 26);
